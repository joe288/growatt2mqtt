...
DalyBms::DalyBms(int rx, int tx, int de, int re)
{
    soft_rx = rx;
    soft_tx = tx;
    de_pin = de;
...
DalyBms::DalyBms(SoftwareSerial* serialPtr, int de, int re)
{
    soft_rx = -1;  // Signal: kein begin() mit Pins aufrufen
    soft_tx = -1;
    de_pin = de;
    re_pin = re;
    this->my_serialIntf = serialPtr;
}
...
bool DalyBms::Init()
{
    ...
    if (soft_rx != -1) {
        ...
        this->my_serialIntf->setTimeout(100); // Set timeout to 100ms for readBytes
    }

    // Initialize RS485 pins if provided
    if (de_pin != -1 && re_pin != -1) {
        pinMode(de_pin, OUTPUT);
        pinMode(re_pin, OUTPUT);
        digitalWrite(de_pin, LOW);  // DE low for receive
        digitalWrite(re_pin, LOW);  // RE low for receive (active low)
    }
...
void DalyBms::setRX()
{
    if (de_pin != -1 && re_pin != -1) {
        ...
        digitalWrite(re_pin, LOW); // For MAX485: RE LOW for receive
    }
}
...
{
    //  Serial1.println("<DALY-BMS DEBUG> RX Buffer: [");
    // BMS_DEBUG_WEB("<DALY-BMS DEBUG> RX Buffer: [");
    for (int i = 0; i < XFER_BUFFER_LENGTH; i++)
    {
        // Serial1.print(",0x" + (String)this->my_rxBuffer[i]);
